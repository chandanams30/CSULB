...
namespace ThoughtFocus.Common.Workflow.Core.Bus
{
    ...
    public sealed class ExecutionRequestParameters
    {
        ...
        public ConditionTypeEnumeration ConditionType { get;  set; }
        ...
        public long TransitionDefinationID { get; set; }


        public static ExecutionRequestParameters Create(long processId,long workFlowID, IEnumerable<ParameterDefinitionWithValue> parameters, TransitionDefinition transition)
        {
            return Create( processId, workFlowID , parameters,  transition, false);
        }

        public static ExecutionRequestParameters Create(long processId,long workFlowID,IEnumerable<ParameterDefinitionWithValue> parameters, TransitionDefinition transition, bool isPreExecution)
        {
            var ret = Create(processId,workFlowID, parameters, transition.To, transition.Condition, isPreExecution);
            ret.TransitionName = transition.Name;
            ret.ActivityName = transition.To.Name;
            ret.TransitionDefinationID = transition.ID;
            return ret;
        }

        public static ExecutionRequestParameters Create(long processId,long workFlowID ,IEnumerable<ParameterDefinitionWithValue> parameters, ActivityDefinition activityToExecute, ConditionDefinition condition)
        {
            return Create(processId,workFlowID, parameters, activityToExecute, condition, false);
        }

        public static ExecutionRequestParameters Create(long processId,long workFlowID, IEnumerable<ParameterDefinitionWithValue> parameters, ActivityDefinition activityToExecute, ConditionDefinition condition, bool isPreExecution)
        {
           ...
           // var methods = new List<MethodToExecuteInfo>(implementation.Count);
            var methods = new List<MethodToExecuteInfo>();
            var executionParameters = new ExecutionRequestParameters
                                          {
                                              ...
                                              ConditionMethod = condition.ConditionType == ConditionTypeEnumeration.Action ? GetMethodToExecuteInfo(parametersList, ActionDefinition.Create(condition.Action, 0)) : null,
                                          };
        ...
        {
            var inputParameters = new List<MethodParameterInfo>(action.ActionDefinition.InputParameters.Count());
            var outputParameters = new List<MethodParameterInfo>(action.ActionDefinition.OutputParameters.Count());
            var method = new MethodToExecuteInfo
                             {
                                 ...
                                 Order = action.Order,
                                 ActionName = action.ActionDefinition.Name
                             };
                    ...
                    new MethodParameterInfo
                        {
                            ...
                            Name = inParameter.ParameterDefinition.Name,
                            Type = inParameter.ParameterDefinition.Type,
                            //TODO - Десериализация
                            DefaultValue = inParameter.ParameterDefinition.SerializedDefaultValue
                        }));
                    ...
                    new MethodParameterInfo
                        {
                            ...
                            Name = outParameter.ParameterDefinition.Name,
