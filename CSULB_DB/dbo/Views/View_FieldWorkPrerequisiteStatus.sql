...
FROM (
	SELECT FW.ID AS FieldWorkID
		,CASE 
			WHEN  FWD.[isRequiredPrerequisite] = 1  
			...
			WHEN FWD.[isRequiredPrerequisite] = 1   
				...
				AND A.[IsApproved] = 1
				THEN 2
			WHEN FWD.[isRequiredPrerequisite] = 0  
				THEN 3
			--ELSE 1
			END AS [STATUS]
	FROM [FieldWork].[FieldWork] FW
	LEFT JOIN [Master].[FieldWorkCourses] FWC ON FWC.ID =  FW.FieldWorkCourseID 
	JOIN [Master].[CourseTerm] CT ON FWC.CourseTermID = CT.ID
	LEFT JOIN [Master].[FieldWorkDocuments] FWD ON FWD.CourseID = CT.CourseID
	LEFT JOIN [FieldWork].[Attachments] A ON A.DocumentID = FWD.DocumentID
		AND A.UserID = FW.UserID 
	) AS T 
GROUP BY [FieldWorkID]