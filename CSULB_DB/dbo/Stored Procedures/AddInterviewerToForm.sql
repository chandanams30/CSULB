...
BEGIN
	...
	IF NOT EXISTS (SELECT * FROM [Application].[Interviewer] WHERE [FormID]=@FormID)
    BEGIN
	INSERT INTO [Application].[Interviewer]
           ([FormID]
           ,[InterviewerUserID])
     VALUES
           (@FormID
	 ...
	 DECLARE @InterviewID AS BIGINT
		...
		BEGIN
			...
			INSERT INTO [Application].[InterviewerAttachments]
				([InterviewID]
				,[DocumentID])
				VALUES
				(@InterviewID,27) --27	Interview Rating Sheet 
		END
