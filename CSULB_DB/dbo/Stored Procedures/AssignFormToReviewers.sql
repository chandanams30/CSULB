...
WHERE F.[FormStateID]=3 AND Rev.[FormID] IS NULL --AND F.[ProgramID]=8 --AND F.[ID]=86
...
WHILE @i <= @formCount
	BEGIN
		...
		DECLARE @FormID as Bigint, @ProgramID AS BIGINT
		SELECT @FormID=FormID, @ProgramID = ProgramID FROM #tempUnAssignedForms WHERE row_num=@i;
		
		SELECT @TopCount=[AssignFormToReviewersCount] FROM [Master].[ProgramConfigurations] WHERE [ProgramID] = @ProgramID;

		--REVIEWERS LIST FOR THE FROM PROGRAM
		select PU.UserID AS [ReviewerID], PU.[ProgramID], pu.[RoleID]  INTO #tempReviewers FROM [Master].[ProgramUsers] PU WHERE PU.[ProgramID]=@ProgramID AND PU.[RoleID]=6
